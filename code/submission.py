...
HHH4_COMMIT_MSG = f"Add KIT-hhh4 forecasts for {forecast_date}"
...
# --- Generate nowcasts and hhh4 forecasts ---
subprocess.run(
    [
        RSCRIPT,
        "--vanilla",
...
if repo.is_dirty():
    repo.index.commit(HHH4_COMMIT_MSG)
    repo.remote("origin").push()
    print("✅ Committed and pushed hhh4 forecasts to RESPINOW-realtime repository.")
else:
    print("⚠️ No new or modified forecast files detected — nothing to commit.")


# --- Submit nowcasts to Hub (PyGithub) ---
...
# --- Submit hhh4 forecasts to Hub (PyGithub) ---
auth = Auth.Token(os.environ["GITHUB_TOKEN"]) # PAT respinow-realtime
g = Github(auth=auth)
...
# create temp branch in your fork
base_ref = dst_repo.get_git_ref(f"heads/{BASE}")
dst_repo.create_git_ref(ref=f"refs/heads/{HHH4_BRANCH}", sha=base_ref.object.sha)

# upload CSVs
file = ROOT / f"forecasts/hhh4-coupling/{forecast_date}-icosari-sari-hhh4-coupling.csv"
dest_path = f"submissions/icosari/sari/KIT-hhh4/{forecast_date}-icosari-sari-KIT-hhh4.csv"
content = file.read_text(encoding="utf-8")

dst_repo.create_file(
    path=dest_path,
    message=HHH4_COMMIT_MSG,
    content=content,
    branch=HHH4_BRANCH,
)
...
body = (
    ...
    f"the **KIT-hhh4** model."
)

pr = upstream.create_pull(
    title=f"Add KIT-hhh4 forecasts for {forecast_date}",
    body=body,
    head=f"{dst_repo.owner.login}:{HHH4_BRANCH}",
    base=BASE,
)
print(f"✅ PR created for hhh4 forecasts: {pr.html_url}")
